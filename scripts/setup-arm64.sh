...
download_components() {
    ...
    if [ ! -f "kubebuilder/bin/etcd" ]; then
        echo "Downloading kubebuilder tools for ARM64..."
    ...
    if [ ! -f "kubebuilder/bin/kubelet" ]; then
        echo "Downloading kubelet for ARM64..."
    ...
    if [ ! -d "/opt/cni" ]; then
        ...
        echo "Installing containerd for ARM64..."
        wget https://github.com/containerd/containerd/releases/download/v2.1.2/containerd-static-2.1.2-linux-arm64.tar.gz -O /tmp/containerd.tar.gz
        sudo tar zxf /tmp/containerd.tar.gz -C /opt/cni/
        rm /tmp/containerd.tar.gz

        echo "Installing runc for ARM64..."
        sudo curl -L "https://github.com/opencontainers/runc/releases/download/v1.2.6/runc.arm64" -o /opt/cni/bin/runc

        echo "Installing CNI plugins for ARM64..."
    ...
    if [ ! -f "kubebuilder/bin/kube-controller-manager" ]; then
        echo "Downloading additional components for ARM64..."
